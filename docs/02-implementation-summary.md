...
- `app`은 HTTP 요청을 각 모듈 public API로 연결한다.
- `platform`은 credential 검증, 원격 API 호출, GitHub/GitLab adapter 선택을 담당한다.
- `connection`은 사용자, 플랫폼 연결, PAT 암호화, 세션 상태를 담당한다.
- `repository`, `issue`, `comment`는 각자 캐시와 업무 유스케이스를 소유한다.
- `shared-kernel`은 동기화 상태와 공통 예외를 제공한다.

## 3. 반영 완료된 개선 사항

- GitHub 전용 경로와 식별자 중심 구조를 플랫폼 공통 API 기준으로 전환했다.
- PAT 검증과 저장 흐름을 platform / connection 책임으로 분리했다.
- 원격 API 호출을 `PlatformRemoteFacade` 뒤로 모았다.
- repository / issue / comment 모듈의 connection 직접 의존을 제거했다.
